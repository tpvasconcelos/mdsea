...
class _BaseSimulator:
    ...
    def get_dists(self, radius: Optional[float],
                  where: str = 'inside',
...
class ContinuousPotentialSolver(_BaseSimulator):
    ...
    def track_energies(self):
        # Potential energies
        self.mean_epot = np.add.reduce(
            self.sm.POT.potential(self.distances)) / self.sm.NUM_PARTICLES
        # Kinetic energies
