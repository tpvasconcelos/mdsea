...
class VelGen(_Gen):
    ...
    def mb(self, mass, temp, k_boltzmann) -> np.ndarray:
        ...
        def cartesian_coords(i, r):
            print("\n", i)
            
            if i == self.ndim - 1:
                print("--1")
                return r * np.cos(thetas[-1]) \
                       * np.prod([np.sin(thetas[i])
                                  for i in range(0, self.ndim - 3)])
            
            elif i == self.ndim - 2:
                print("--2")
                return r * np.sin(thetas[-1]) \
                       * np.prod([np.sin(thetas[i])
                                  for i in range(0, self.ndim - 3)])
            
            elif i == 0:
                print("--3")
                return r * np.cos(thetas[0])
            
            else:
                print("--4")
                return r * np.cos(thetas[i]) \
                       * np.prod([np.sin(thetas[i])
                                  for i in range(0, i - 2)])
        
        if temp == 0:
            self.generated = True
            return np.zeros((self.ndim, self.nparticles),
                            dtype=DTYPE)
        ...
        self.coords = np.array([cartesian_coords(i, self.speeds)
...
class CoordGen(_Gen):
    ...
    def random(self, pradius) -> np.ndarray:
        ...
        self.coords = pradius + us * np.random.random((self.ndim,
                                                       self.nparticles))
