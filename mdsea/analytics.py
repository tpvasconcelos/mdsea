...
def make_mp4(fname: str, stills_dir: str, fps: float, timeit: bool = False) -> None:
...
class Analyser:
    def __init__(self, sm: SysManager) -> None:
        ...
        self.r_vecs = np.stack((self.r_coords[:, i] for i in range(self.sm.NDIM)), axis=-1)

        # TODO(tpvasconcelos) explain the difference between v_coords & v_vecs
        self.v_coords = self.sm.get_ds(self.sm.vcoord_dsname)
        self.v_vecs = np.stack((self.v_coords[:, i] for i in range(self.sm.NDIM)), axis=-1)
