...
def glasswalls(
    ...
    wall_bottom = {
        "rotation": (0, 0, 0),
        "location": (len_box / 2, len_box / 2, -thickness / 2),
        "dimensions": (len_box + 2 * thickness, len_box + 2 * thickness, thickness),
    }

    wall_top = {
        "rotation": (0, 0, 0),
        "location": (len_box / 2, len_box / 2, len_box + (thickness / 2)),
        "dimensions": (len_box + 2 * thickness, len_box + 2 * thickness, thickness),
    }

    wall_front = {
        "rotation": (math.radians(90), 0, 0),
        "location": (len_box / 2, -thickness / 2, len_box / 2),
        "dimensions": (len_box, len_box, thickness),
    }

    wall_back = {
        "rotation": (math.radians(90), 0, 0),
        "location": (len_box / 2, len_box + thickness / 2, len_box / 2),
        "dimensions": (len_box, len_box, thickness),
    }

    wall_left = {
        "rotation": (0, math.radians(90), 0),
        "location": (-thickness / 2, len_box / 2, len_box / 2),
        "dimensions": (len_box, len_box + 2 * thickness, thickness),
    }

    wall_right = {
        "rotation": (0, math.radians(90), 0),
        "location": (len_box + (thickness / 2), len_box / 2, len_box / 2),
        "dimensions": (len_box, len_box + 2 * thickness, thickness),
    }
    ...
    for i, wall in enumerate(walls):
        bpy.ops.mesh.primitive_cube_add(location=wall["location"], rotation=wall["rotation"])
        obj = bpy.context.object
        obj.name = f"Wall-{i}"
