...
class SysManager:
    def __init__(
        ...
        self.QUENCH_STEP = [round(self.STEPS * t) for t in self.QUENCH_TIMING]
        ...
        self.oned_dsnames = [self.potenergy_dsname, self.kinenergy_dsname, self.temp_dsname]
    ...
    def _create_tree(self) -> None:
        ...
        if not os.path.exists(dir_simfiles):
            ...
            log.debug("Couldn't find a 'simulations-folder'. " "A new one was created: %s", dir_simfiles)

        # Create simulation directory tree
        for directory in self.dir_tree:
            if os.path.exists(directory):
                log.warning("Directory exists and will be overwritten: %s", directory)
    ...
    def _close_datafile(self):
        ...
        try:
            ...
            log.warning("You tried to close a data file " "(%s) that was already closed.", self.data_path)

    def _init_datasets(self) -> None:
        """ Create datasets for a given h5py.File. """

        if self.dfile is None:
            raise FileNotFoundError(f"Datafile does not exist! " f"Expected at: {self.data_path}")

        # Datasets with shape=(STEPS, NDIM, NUM_PARTICLES)
        for name in self.coord_dsnames:
            self.dfile.create_dataset(name, dtype=DTYPE, shape=(self.STEPS, self.NDIM, self.NUM_PARTICLES))

        # Datasets with shape=(STEPS,)
        for name in self.oned_dsnames:
            self.dfile.create_dataset(name, dtype=DTYPE, shape=(self.STEPS,))
    ...
    @classmethod
    def new(cls, simid: Optional[str] = None, initfilesys: bool = True, **kwargs) -> "SysManager":
        """ Create a new System Manager instance. """
        if simid is None:
            simid = _gen_newid()
        path_sim = cfm.SIM_PATH.format(simid)
        records = {"id": simid, "path": path_sim}
    ...
    def load(cls, simid: str) -> "SysManager":
        ...
        if not all(paths_exist):
            records = {"id": simid, "path": path_sim, "settings": path_settings}
    ...
    def initfilesys(self) -> None:
        ...
        if not self.new_sim:
            records = {"sim ID": self.SIM_ID, "new simulation": self.new_sim}
            msg = (
                f"You were about to initialize an existing simulation, "
    ...
    def update_ds(self, dsname: str, x: Union[list, np.ndarray], i: int) -> None:
        ...
        try:
            ...
            if self.dfile is None:
                msg = "The dataset's datafile you are trying to update " "might be closed or not exist."
            ...
            msg = (
                "A ValueError ('%s') was caught while trying to update a "
